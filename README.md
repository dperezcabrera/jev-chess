...
**Tournament** in the top bar opens a second page, `/tournament`, with the same board and panel, so the single-game screen stays as it is. Pick the players (Jev, Laya and the LLMs you added, plus yourself if you tick "I play too"), the number of rounds and the clock (30 minutes, 1 hour, 2 hours of deciding time per player and game, or none), and the tournament runs by the Swiss system: a player whose deciding time passes the limit loses on time, so no game runs for ever, and with a clock the player bars and the small boards count down. On your own board, **Take back** undoes your last move and the reply it got, or, while you replay the game, takes it back to the position you are looking at; the moves taken back disappear from the record with their cost, tokens and time, as if they had never been played. **Pause my clock** stops your time while you are away without touching the other boards; it runs again when you play or press it again. every round pairs players on equal scores, nobody meets twice while that can be avoided, colours alternate as far as the balance allows, and with an odd number of players the lowest-placed one who has not had a bye sits the round out and scores a full point. Every board of a round is its own game: the server plays the games between models at the same time, up to `TOURNAMENT_CONCURRENCY` model calls in flight (3 by default), and a game with you waits for your moves; when every board of the round is over the next round is paired. Under the big board a grid of small boards shows the whole round at a glance, each with its players, a clock per side counting the time spent deciding, yours included (ticking while a model thinks or while you think) and its state: playing, your move, the result, or stopped by a gateway error, which a second click retries; a board lost by illegal moves can be let go on with a second click too, while its round is still being played: the forfeit leaves the standings and the game continues from the same position with a clean count. Click a small board to follow it on the big one; your own board is selected when it is your turn. The panel shows the round, a progress bar, the elapsed time and the standings in short, **Pause** and **Play** to stop the models and let them go on (the move in flight is asked again), **Add a round** to extend the tournament by one round, even after it finished, and a round navigator (arrows, or Page Up and Page Down) that puts any earlier round on the grid with its results and a line of statistics (boards, finished, decisive, bye, thinking time, cost), each small board opening its game to replay and analyze; **Standings** opens the full table (games, wins, draws, losses, points, the tie-breaks in the order they decide: Buchholz Cut 1, Buchholz, Buchholz Cut 2, Sonneborn-Berger and wins, then calls, tokens, deciding time, illegal answers and cost), **Rounds** every board of every round with white, black, result and byes, each one a click away from the big board to replay and analyze it with the engine, **PGN** downloads every finished game of the tournament in one file (with `Round` tags), and **Data** downloads everything recorded as JSON: participants, rounds, each game move by move with who decided, tokens, seconds, cost, illegal answers and the probabilities a System One model gave, results, standings and tie-breaks, ready for an article. Every tournament game also counts in the session ranking. The whole tournament is saved to `tournaments/<id>.json` after every move (positions, moves, what each side spent, illegal answers, PGN and record of each game, pairings and standings), so a restart loses nothing: the start dialog lists the saved tournaments to resume the unfinished ones or review the finished ones, and `TOURNAMENT_DIR` moves the folder; with Docker, mount it: `-v ./tournaments:/app/tournaments`.
...
| POST | `/api/tournament/board/{n}/rewind` | `{"plies": 2}` | Take moves back on board `n` of the current round, finished or not (0 reopens it where it stands), and play it on: the game leaves the standings until it ends again; earlier rounds cannot be touched |
...
| POST | `/api/tournament/rounds` | | One more round, for a running tournament or one that just finished |
| POST | `/api/tournament/pause`, `/api/tournament/play` | | Pause the tournament (the models stop being asked, positions and clocks stay) and play on; a paused tournament resumes paused after a restart |
| POST | `/api/tournament/participants` | `{"participants": ["llm:openai/gpt-6-astra"]}` | Let players into a running tournament: newcomers pair among themselves on extra boards of the current round, or with the player holding the bye, and join the Swiss pairing from the next round |
...
| POST | `/api/takeback` | `{"ply": 52}` optional | Take your last move back with the model's reply, or go back to a position where it is your move; the moves taken back leave the record and the totals |
